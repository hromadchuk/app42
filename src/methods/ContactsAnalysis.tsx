...
function SectionBlock({
    ...
    header: string;
    showAll: string;
    showAllCallback: (data: IShowAllOptions) => void;
}) {
    if (!users.length) {
        return null;
    }
    ...
    return (
        <Section
            header={
                <Section.Header>
                    {header}
                    <Caption level="1" weight="3" style={{ marginLeft: 5, color: 'var(--tgui--secondary_hint_color)' }}>
                        {users.length}
                    </Caption>
                </Section.Header>
            }
...
export default function ContactsAnalysis() {
    ...
    const blocks = [
        {
            type: EFilerType.NON_MUTUAL,
            users: nonMutualUsersData,
            lang: 'non_mutual'
        },
        {
            type: EFilerType.MUTUAL,
            users: mutualUsersData,
            lang: 'mutual'
        },
        {
            type: EFilerType.PREMIUM,
            users: premiumUsersData,
            lang: 'premium'
        },
        {
            type: EFilerType.WITHOUT_PHOTO,
            users: withoutPhotoUsersData,
            lang: 'without_photo'
        },
        {
            type: EFilerType.VERIFIED,
            users: verifiedUsersData,
            lang: 'verified'
        },
        {
            type: EFilerType.WITHOUT_PHONE,
            users: withoutPhoneUsersData,
            lang: 'without_phone'
        },
        {
            type: EFilerType.WITHOUT_USERNAME,
            users: withoutUsernameUsersData,
            lang: 'without_username'
        },
        {
            type: EFilerType.DELETED,
            users: deletedUsersData,
            lang: 'deleted'
        },
        {
            type: EFilerType.LONG_TIME_ONLINE,
            users: longTimeOnlineUsersData,
            lang: 'long_time_online'
        },
        {
            type: EFilerType.RECENT_ONLINE,
            users: recentOnlineUsersData,
            lang: 'recent_online'
        },
        {
            type: EFilerType.FEW_USERNAMES,
            users: fewUsernamesUsersData,
            lang: 'few_usernames'
        },
        {
            type: EFilerType.HIDE_ONLINE,
            users: hideOnlineUsersData,
            lang: 'hide_online'
        },
        {
            type: EFilerType.OLD,
        ...
        <>
            {blocks.map((block) => (
                <SectionBlock
                    ...
                    type={block.type}
                    users={block.users}
                    header={mt(`headers.${block.lang}`)}
                    showAll={mt('show_all_users')}
                    showAllCallback={({ users, type, header }) => {
                        setModalUsers(users);
