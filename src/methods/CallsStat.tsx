...
interface IUsersContent {
    selectedTab: ETabId;
    stat: IStatResult;
    callsDecline: string[];
    callback: (row: IUserTop) => void;
}

function UsersContent({ selectedTab, stat, callsDecline, callback }: IUsersContent) {
    if (!stat) return null;
    ...
    const getDescription = (row: IUserTop): string => {
        ...
        return declineAndFormat(row.calls, callsDecline);
    };
